...
MFUN(pluck_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_pluck_init(ug->sp, ug->osc, ifreq);
}
...
MFUN(tenvx_set_hold)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->hold = hold);
}
...
MFUN(sdelay_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_sdelay_init(ug->sp, ug->osc, size);
}
...
MFUN(count_set_count)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->count = count);
}
...
MFUN(count_set_mode)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->mode = mode);
}
...
MFUN(drip_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_drip_init(ug->sp, ug->osc, dettack);
}
...
MFUN(drip_set_num_tubes)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->num_tubes = num_tubes);
}
...
MFUN(drip_set_damp)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->damp = damp);
}
...
MFUN(drip_set_shake_max)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->shake_max = shake_max);
}
...
MFUN(drip_set_freq1)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->freq1 = freq1);
}
...
MFUN(drip_set_freq2)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->freq2 = freq2);
}
...
MFUN(autowah_set_level)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->level = level);
}
...
MFUN(tabread_init)
{
	...
	m_float mode = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_tabread_init(ug->sp, ug->osc, ft, mode);
}
...
MFUN(tabread_set_index)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->index = index);
}
...
MFUN(tabread_set_offset)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->offset = offset);
}
...
MFUN(tabread_set_wrap)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->wrap = wrap);
}
...
MFUN(delay_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_delay_init(ug->sp, ug->osc, time);
}
...
MFUN(delay_set_feedback)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->feedback = feedback);
}
...
MFUN(port_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_port_init(ug->sp, ug->osc, htime);
}
...
MFUN(vdelay_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_vdelay_init(ug->sp, ug->osc, maxdel);
}
...
MFUN(bitcrush_set_bitdepth)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->bitdepth = bitdepth);
}
...
MFUN(bitcrush_set_srate)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->srate = srate);
}
...
MFUN(fold_set_incr)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->incr = incr);
}
...
MFUN(pan2_set_type)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->type = type);
}
...
MFUN(rpt_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_rpt_init(ug->sp, ug->osc, maxdur);
}
...
MFUN(dtrig_set_loop)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->loop = loop);
}

MFUN(dtrig_get_delay)
{
	GW_dtrig* ug = (GW_dtrig*)o->ugen->ug;
	RETURN->v_float = ug->osc->delay;
}
...
MFUN(dtrig_set_delay)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->delay = delay);
}
...
MFUN(dtrig_set_scale)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->scale = scale);
}
...
MFUN(comb_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_comb_init(ug->sp, ug->osc, looptime);
}
...
MFUN(comb_set_revtime)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->revtime = revtime);
}
...
MFUN(allpass_init)
{
	...
	m_float looptime = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_allpass_init(ug->sp, ug->osc, looptime);
}

MFUN(allpass_looptime)
{
	m_uint gw_offset = SZ_INT;
	GW_allpass* ug = (GW_allpass*)o->ugen->ug;
	m_float looptime = *(m_float*)(shred->mem + gw_offset);
...
MFUN(tbvcf_set_dist)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->dist = dist);
}
...
MFUN(tbvcf_set_asym)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->asym = asym);
}
...
MFUN(lpf18_set_cutoff)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->cutoff = cutoff);
}
...
MFUN(phaser_set_MaxNotch1Freq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->MaxNotch1Freq = MaxNotch1Freq);
}
...
MFUN(phaser_set_MinNotch1Freq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->MinNotch1Freq = MinNotch1Freq);
}
...
MFUN(phaser_set_Notch_width)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->Notch_width = Notch_width);
}
...
MFUN(phaser_set_NotchFreq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->NotchFreq = NotchFreq);
}
...
MFUN(phaser_set_VibratoMode)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->VibratoMode = VibratoMode);
}
...
MFUN(phaser_set_depth)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->depth = depth);
}
...
MFUN(phaser_set_feedback_gain)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->feedback_gain = feedback_gain);
}
...
MFUN(phaser_set_invert)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->invert = invert);
}
...
MFUN(phaser_set_lfobpm)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->lfobpm = lfobpm);
}
...
MFUN(dmetro_set_time)
{
	...
	m_float time = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->time = time);
}
...
MFUN(oscmorph_init)
{
	...
	gw_offset += SZ_INT;
	m_uint tbl_iter;
	sp_ftbl* tbl[m_vector_size(tbl_ptr->array)];
	for(tbl_iter = 0; tbl_iter < m_vector_size(tbl_ptr->array); tbl_iter++)
		tbl[tbl_iter] = FTBL((M_Object)i_vector_at(tbl_ptr->array, tbl_iter));
	m_int nft = *(m_int*)(shred->mem + gw_offset);
	gw_offset += SZ_INT;
	m_float phase = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_oscmorph_init(ug->sp, ug->osc, tbl, nft, phase);
}
...
MFUN(oscmorph_set_wtpos)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->wtpos = wtpos);
}
...
MFUN(wpkorg35_set_saturation)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->saturation = saturation);
}
...
MFUN(reverse_init)
{
	...
	m_float delay = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_reverse_init(ug->sp, ug->osc, delay);
}
...
MFUN(posc3_init)
{
	...
	sp_posc3_init(ug->sp, ug->osc, tbl);
}
...
MFUN(waveset_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_waveset_init(ug->sp, ug->osc, ilen);
}
...
MFUN(pitchamdf_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_pitchamdf_init(ug->sp, ug->osc, min, max);
}
...
MFUN(zitarev_set_in_delay)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->in_delay = in_delay);
}
...
MFUN(zitarev_set_lf_x)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->lf_x = lf_x);
}
...
MFUN(zitarev_set_rt60_low)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->rt60_low = rt60_low);
}
...
MFUN(zitarev_set_rt60_mid)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->rt60_mid = rt60_mid);
}
...
MFUN(zitarev_set_hf_damping)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->hf_damping = hf_damping);
}
...
MFUN(zitarev_set_eq1_freq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->eq1_freq = eq1_freq);
}
...
MFUN(zitarev_set_eq1_level)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->eq1_level = eq1_level);
}
...
MFUN(zitarev_set_eq2_freq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->eq2_freq = eq2_freq);
}
...
MFUN(zitarev_set_eq2_level)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->eq2_level = eq2_level);
}
...
MFUN(maygate_set_prob)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->prob = prob);
}
...
MFUN(maygate_set_mode)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->mode = mode);
}
...
MFUN(jitter_set_cpsMin)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->cpsMin = cpsMin);
}
...
MFUN(jitter_set_cpsMax)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->cpsMax = cpsMax);
}
...
MFUN(eqfil_set_gain)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->gain = gain);
}
...
MFUN(expon_set_a)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->a = a);
}

MFUN(expon_get_dur)
{
	GW_expon* ug = (GW_expon*)o->ugen->ug;
	RETURN->v_float = ug->osc->dur;
}
...
MFUN(pdhalf_set_amount)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->amount = amount);
}
...
MFUN(mincer_init)
{
	...
	gw_offset+=SZ_INT;
	sp_ftbl* ft = FTBL(ft_obj);
	sp_mincer_init(ug->sp, ug->osc, ft);
}
...
MFUN(mincer_set_pitch)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->pitch = pitch);
}
...
MFUN(peaklim_set_thresh)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->thresh = thresh);
}
...
MFUN(tblrec_init)
{
	...
	sp_tblrec_init(ug->sp, ug->osc, bar);
}
...
MFUN(tseq_set_shuf)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->shuf = shuf);
}
...
MFUN(streson_set_fdbgain)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->fdbgain = fdbgain);
}
...
MFUN(ptrack_init)
{
	...
	gw_offset += SZ_INT;
	sp_ptrack_init(ug->sp, ug->osc, ihopsize, ipeaks);
}
...
MFUN(diskin_init)
{
	...
	gw_offset += SZ_INT;
	m_str filename = STRING(filename_obj);
	sp_diskin_init(ug->sp, ug->osc, filename);
}

MFUN(diskin_filename)
{
	m_uint gw_offset = SZ_INT;
	GW_diskin* ug = (GW_diskin*)o->ugen->ug;
	M_Object filename_obj = *(M_Object*)(shred->mem + gw_offset);
...
MFUN(fosc_set_indx)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->indx = indx);
}
...
MFUN(tseg_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_tseg_init(ug->sp, ug->osc, ibeg);
}
...
MFUN(tseg_set_type)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->type = type);
}
...
MFUN(gbuzz_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_gbuzz_init(ug->sp, ug->osc, ft, iphs);
}
...
MFUN(gbuzz_set_nharm)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->nharm = nharm);
}
...
MFUN(gbuzz_set_lharm)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->lharm = lharm);
}
...
MFUN(paulstretch_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_paulstretch_init(ug->sp, ug->osc, ft, windowsize, stretch);
}
...
MFUN(conv_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_conv_init(ug->sp, ug->osc, ft, iPartLen);
}
...
MFUN(incr_set_step)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->step = step);
}
...
MFUN(slice_init)
{
	...
	sp_slice_init(ug->sp, ug->osc, vals, buf);
}

MFUN(slice_get_id)
{
	GW_slice* ug = (GW_slice*)o->ugen->ug;
	RETURN->v_float = ug->osc->id;
}
...
MFUN(compressor_set_ratio)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->ratio = ratio);
}
...
MFUN(dist_set_pregain)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->pregain = pregain);
}
...
MFUN(dist_set_postgain)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->postgain = postgain);
}
...
MFUN(dist_set_shape1)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->shape1 = shape1);
}
...
MFUN(dist_set_shape2)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->shape2 = shape2);
}
...
MFUN(progress_set_nbars)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->nbars = nbars);
}

MFUN(progress_get_skip)
{
	GW_progress* ug = (GW_progress*)o->ugen->ug;
	RETURN->v_uint = ug->osc->skip;
}
...
MFUN(progress_set_skip)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->skip = skip);
}
...
MFUN(blsquare_set_width)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->width = width);
}
...
MFUN(nsmp_init)
{
	...
	sp_nsmp_init(ug->sp, ug->osc, ft, sr, init);
}
...
MFUN(nsmp_set_index)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->index = index);
}
...
MFUN(pshift_set_shift)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->shift = shift);
}
...
MFUN(pshift_set_window)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->window = window);
}
...
MFUN(pshift_set_xfade)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->xfade = xfade);
}
...
MFUN(vocoder_set_bwratio)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (*ug->osc->bwratio = bwratio);
}
...
MFUN(phasor_init)
{
	...
	m_float iphs = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_phasor_init(ug->sp, ug->osc, iphs);
}
...
MFUN(osc_init)
{
	...
	m_float phase = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_osc_init(ug->sp, ug->osc, tbl, phase);
}
...
MFUN(fog_init)
{
	...
	m_float iphs = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_fog_init(ug->sp, ug->osc, wav, win, iolaps, iphs);
}
...
MFUN(fog_set_dens)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->dens = dens);
}
...
MFUN(fog_set_trans)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->trans = trans);
}
...
MFUN(fog_set_band)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->band = band);
}

MFUN(fog_get_ris)
{
	GW_fog* ug = (GW_fog*)o->ugen->ug;
	RETURN->v_float = ug->osc->ris;
}
...
MFUN(smoothdelay_init)
{
	...
	gw_offset += SZ_INT;
	sp_smoothdelay_init(ug->sp, ug->osc, maxdel, interp);
}
...
MFUN(fof_init)
{
	...
	m_float iphs = *(m_float*)(shred->mem + gw_offset);
	gw_offset += SZ_FLOAT;
	sp_fof_init(ug->sp, ug->osc, sine, win, iolaps, iphs);
}
...
MFUN(fof_set_fund)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->fund = fund);
}
...
MFUN(fof_set_form)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->form = form);
}
...
MFUN(dust_set_density)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->density = density);
}
...
MFUN(dust_set_bipolar)
{
	...
	gw_offset += SZ_INT;
	RETURN->v_uint = (ug->osc->bipolar = bipolar);
}
...
MFUN(revsc_set_lpfreq)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->lpfreq = lpfreq);
}
...
MFUN(bar_init)
{
	...
	gw_offset += SZ_FLOAT;
	sp_bar_init(ug->sp, ug->osc, iK, ib);
}
...
MFUN(bar_set_bcR)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->bcR = bcR);
}

MFUN(bar_get_T30)
{
	GW_bar* ug = (GW_bar*)o->ugen->ug;
	RETURN->v_float = ug->osc->T30;
}
...
MFUN(bar_set_scan)
{
	...
	gw_offset += SZ_FLOAT;
	RETURN->v_float = (ug->osc->scan = scan);
}
...
struct Type_ t_foo = {"Foo", SZ_INT, &t_ugen};
...
env->curr->doc = "soudpipe float array type
...
env->class_def->doc = "Casio-style phase distortion with \"pivot point\" on the X axisThis module is designed to emulate the classic phase distortion synthesis technique.From the mid 90's. The technique reads the first and second halves of the ftblat different rates in order to warp the waveform. For example, pdhalf can smoothly transition a sinewave into something approximating a sawtooth wave.";
	...
	fun = new_DL_Func("void", "ft", (m_uint)mincer_init);
		arg = dl_func_add_arg(fun, "ftbl", "ft");
