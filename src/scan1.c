...
static m_bool scan1_Func_Call( Env env, Func_Call* func_call )
{
  ...
  if(func_call->types)
  {
    ...
    while(list)
    {
/*      Type t = namespace_lookup_type(env->curr, list->xid, 1); // was find_type*/
      Type t = find_type(env, list->list); // was find_type
      if(!t)
      {
...
static m_bool scan1_Enum(Env env, Stmt_Enum stmt)
{
  ...
  if(stmt->xid)
  {
    if(namespace_lookup_type(nspc, stmt->xid, 0))
    {
...
static m_bool scan1_Stmt(Env env, Stmt* stmt)
{
  ...
  if(!t)
  {
    err_msg(SCAN1_, l->self->pos, "type '%s' unknown in declaration ", S_name(l->self->type->xid->xid));
    return -1;
  }
  while(list)
  {
    ...
    if(var_decl->array)
    {
      ...
      if(var_decl->array->exp_list)
      {
        err_msg(SCAN1_, l->self->pos, "array declartion must be empty in union.");
        return -1;
      }
    }
    list = list->next;
...
// TODO: remove type on errors
  ...
  if(!ptr->ret_type)
  {
    err_msg(SCAN1_, ptr->pos, "invalid type in func ptr declaration %s",  S_name(ptr->xid));
    return -1;
  }

  if(!env && ptr->key == ae_key_static)
  {
    err_msg(SCAN2_, ptr->pos, "can't declare func pointer static outside of a class");
    return -1;
  }
...
/*    err_msg(SCAN1_, f->pos,"scan1: invalid return type %p", S_name(f->type_decl->xid->xid));*/
  ...
  if(f->type_decl->array)
  {
    ...
    {
      err_msg(SCAN1_, f->type_decl->array->pos, "function '%s':", S_name(f->name) );
  ...
  while(body)
  {
    switch(body->section->type)
    {
      case ae_section_stmt:
        ret = scan1_Stmt_List(env, body->section->stmt_list);
      break;
      case ae_section_func:
        ret = scan1_Func_Def(env, body->section->func_def);
        break;
      case ae_section_class:
        ret = scan1_Class_Def(env, body->section->class_def);
      break;
    }
    CHECK_BB(ret)
    body = body->next;
