...
static m_bool emit_pre_constructor_array(Emitter emit, Type type)
{
  ...
  add_instr(emit, Instr_Pre_Ctor_Array_Post);
  return 1;
}
...
/*    add_instr(emit, Assign_Object); // should be assign_func*/
    Instr instr = add_instr(emit, assign_func); // should be assign_func
/*    instr->m_val = */
    return 1;
...
/*  Instr instr = new_Instr();*/
  ...
  // arrays
  if( binary->op == op_shift_left &&
    (binary->lhs->type->array_depth == binary->rhs->type->array_depth + 1)
/*)*/
...
static m_bool emit_Cast_Expression(Emitter emit, Cast_Expression* cast)
{
  ...
  {
      Value v  = namespace_lookup_value(emit->env->curr, insert_symbol(to->name), -1);
      Func f  = namespace_lookup_func(emit->env->curr, insert_symbol(to->name), -1);
    if(cast->exp->exp_type == Primary_Expression_type)
    {
      ...
      Value v2 = namespace_lookup_value(emit->env->curr, insert_symbol(name), -1);
      Func f = namespace_lookup_func(emit->env->curr, insert_symbol(name), -1);
      Instr push = add_instr(emit, Reg_Push_Imm);
...
static m_bool emit_Dur(Emitter emit, Exp_Dur* dur)
{
  ...
  if(isa(dur->base->type, &t_int) > 0)
    add_instr(emit, Cast_i2f);
  CHECK_BB(emit_Expression(emit, dur->unit, 0))
  add_instr(emit, timesf);
  return 1;
}
...
/*m_uint ptr;  */
  if(!func->code)
  { // calling function pointer in func
...
/*  code->m_val = func->code;*/
  ...
  {
    Instr prev = add_instr(emit, Mem_Push_Imm);
/*    prev->m_val = emit->code->stack_depth;  */
  }
...
static m_bool emit_Unary(Emitter emit, Unary_Expression* unary)
{
  ...
  {
    case op_plusplus:
      if(unary->self->meta != ae_meta_var || 
        unary->self->exp_type == Primary_Expression_type &&
        unary->self->primary_exp->value->is_const)  // TODO: check const
      {
    ...
    case op_spork:
      ...
      if(isa(unary->self->type, &t_int) >0)
        add_instr(emit, negate);
      else if(isa(unary->self->type, &t_float) > 0)
        add_instr(emit, negatef);
      else
      {
        err_msg(EMIT_, unary->pos,
...
/*        instr->m_val = e->type->size;*/
    ...
    /*    add_instr(emit, Reg_Push_Imm);*/
/*    add_instr(emit, Reg_Pop_Word4);*/
    Instr instr = add_instr(emit, MkVararg);
    instr->m_val = size;
    instr->m_val2 = (m_uint)kinds;
  }
  printf("func_call->m_func->def->is_variadic %i\n", func_call->m_func->def->is_variadic);
  return 1;
}
...
static m_bool emit_implicit_cast(Emitter emit, Type from, Type to)
{
  ...
  if(from->xid == t_float.xid && to->xid == t_int.xid)
    add_instr(emit, Cast_f2i);
...
static m_bool emit_exp_if(Emitter emit, If_Expression* exp_if )
{
  ...
  ret = emit_Expression(emit, exp_if->cond, 0);
  if(!ret)
      return -1;
...
#endif
/*  m_bool ret = 1;*/
  Instr op = NULL, op2 = NULL, tmp = NULL, tmp2 = NULL;
  f_instr f;
...
static m_bool emit_While(Emitter emit, Stmt_While stmt)
{
  ...
  {
    case te_int:
      tmp = add_instr(emit, Reg_Push_Imm);
      f = Branch_Eq_Int;
      break;
    ...
    case te_dur:
    case te_time:
      tmp = add_instr(emit, Reg_Push_Imm2);
      f = Branch_Eq_Float;
      break;
...
/*        }*/
  ...
  if(emit_Stmt(emit, stmt->body, 1) < 0)
    return -1;
  emit_pop_scope(emit);
  cont_index = vector_size(emit->code->code);
   // emit the action
  if( stmt->c3 )
  {
...
static m_bool emit_Loop(Emitter emit, Stmt_Loop stmt)
{
...
#endif
  m_bool ret = 1;
  Instr init = NULL, op = NULL, deref = NULL, tmp = NULL, dec = NULL, _goto = NULL;
  ...
  frame_push_scope(emit->code->frame);
  // emit the cond
  if(emit_Expression(emit, stmt->cond, 0) < 0)
    return -1;
...
static m_bool emit_Case(Emitter emit, Stmt_Case stmt)
{
  ...
  if(stmt->val->exp_type == Primary_Expression_type)
  {
    ...
    else
    {
      ...
      value = stmt->val->primary_exp->value->is_const == 2 ?
        (m_uint)stmt->val->primary_exp->value->ptr :    // for enum
...
#endif
  Instr instr;
  Decl_List l;Local* local;Var_Decl_List var_list;
  m_bool ret = 1;
  if(!stmt )
    return 1;
  switch(stmt->type)
...
/*          if(exp->exp_type == Primary_Expression_type && exp->primary_exp->type == ae_primary_str)*/
    ...
    case ae_stmt_funcptr:
     ...
     case ae_stmt_union:
      ...
      while(l)
      {
        printf("l->self->value %p %i\n", l->self->list->self->value, local->offset);
if(!emit->env->class_def)
{
  var_list = l->self->list;
  while(var_list)
  {
    var_list->self->value->offset = local->offset;
    var_list = var_list->next;
  }
  switch(stmt->stmt_union->s)
  {
    case SZ_COMPLEX:
      add_instr(emit, Alloc_Word_Complex);
      break;
    case SZ_INT:
      add_instr(emit, Alloc_Word);
      break;
/*    case SZ_FLOAT:*/
/*      add_instr(emit, Alloc_Word_Float);*/
/*      break;*/
  }
}
else
{
  err_msg(EMIT_, l->self->pos, "union not allowed in class def for now");

  return -1;
}
        /*        CHECK_BB(emit_Decl_Expression(emit, l->self))*/
/*        CHECK_BB(emit_Decl_Expression(emit, l->self))*/
/*        switch(stmt->stmt_union->s)*/
/*        {*/
/*          case SZ_INT:*/
...
static m_bool emit_Func_Def(Emitter emit, Func_Def func_def)
{
  ...
  if(func_def->is_variadic)
  {
    ...
    printf("\n%p\n", namespace_lookup_value(emit->env->curr, insert_symbol("vararg"), 0));
  }
