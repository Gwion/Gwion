...
static INSTR(int_post_##name) { *(m_int*)REG(- SZ_INT) = (**(m_int**)REG(- SZ_INT))op; }
...
static INSTR(int_negate) { *(m_int*)REG(-SZ_INT) *= -1; }

INSTR(int_not) { *(m_int*)REG(-SZ_INT) = !*(m_int*)REG(0); }
...
describe(minus,  -,)
describe(times,  *,)
describe(divide, /, TEST0(SZ_INT))
describe(modulo, %, TEST0(SZ_INT))
...
describe_r(minus,  -,)
describe_r(times,  *,)
describe_r(divide, /, TEST0(0))
describe_r(modulo, %, TEST0(0))
describe_r(sl,     <<,)
describe_r(sr,     >>,)
describe_r(sand,   &,)
describe_r(sor,    |,)
