...
GWION_IMPORT(array) {
  ...
  GWI_B(gwi_func_ini(gwi, "T", "random"))
  GWI_B(gwi_func_end(gwi, vm_vector_random, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "A[]", "map:[A]"))
  GWI_B(gwi_func_arg(gwi, "map_t:[A]", "data"))
  GWI_B(gwi_func_end(gwi, vm_vector_map, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "A[]", "compactMap:[A]"))
  GWI_B(gwi_func_arg(gwi, "compactmap_t:[A]", "data"))
  GWI_B(gwi_func_end(gwi, vm_vector_compactmap, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "T[]", "filter"))
  GWI_B(gwi_func_arg(gwi, "filter_t", "data"))
  GWI_B(gwi_func_end(gwi, vm_vector_filter, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "int", "count"))
  GWI_B(gwi_func_arg(gwi, "filter_t", "data"))
  GWI_B(gwi_func_end(gwi, vm_vector_count, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "A", "foldl:[A]"))
  GWI_B(gwi_func_arg(gwi, "fold_t:[A]", "data"))
  GWI_B(gwi_func_arg(gwi, "A", "initial"))
  GWI_B(gwi_func_end(gwi, vm_vector_foldl, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "A", "foldr:[A]"))
  GWI_B(gwi_func_arg(gwi, "fold_t:[A]", "data"))
  GWI_B(gwi_func_arg(gwi, "A", "initial"))
  GWI_B(gwi_func_end(gwi, vm_vector_foldr, ae_flag_none))

  GWI_B(gwi_func_ini(gwi, "auto", "new"))
  GWI_B(gwi_func_arg(gwi, "new_t", "init"))
  GWI_B(gwi_func_end(gwi, vm_vector_new, ae_flag_none))

  GWI_B(gwi_class_end(gwi))

   GWI_B(gwi_oper_ini(gwi, "Array", "Array", NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_at))
   GWI_B(gwi_oper_end(gwi, ":=>", NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_implicit))
//   GWI_B(gwi_oper_end(gwi, "@implicit", NULL))
   GWI_B(gwi_oper_end(gwi, "@implicit", NoOp))
   GWI_B(gwi_oper_ini(gwi, "Array", (m_str)OP_ANY_TYPE, NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_sl))
   GWI_B(gwi_oper_emi(gwi, opem_array_sl))
   GWI_B(gwi_oper_end(gwi, "<<", NULL))
   GWI_B(gwi_oper_ini(gwi, (m_str)OP_ANY_TYPE, "Array", NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_sr))
   GWI_B(gwi_oper_emi(gwi, opem_array_sr))
   GWI_B(gwi_oper_end(gwi, ">>", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", "Array", NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_cast))
   GWI_B(gwi_oper_emi(gwi, opem_array_cast))
   GWI_B(gwi_oper_end(gwi, "$", NULL))
   GWI_B(gwi_oper_ini(gwi, "int", "Array", "int"))
   GWI_B(gwi_oper_add(gwi, opck_array_slice))
   GWI_B(gwi_oper_emi(gwi, opem_array_slice))
   GWI_B(gwi_oper_end(gwi, "[:]", NULL))
   GWI_B(gwi_oper_ini(gwi, "int", "Array", NULL))
   GWI_B(gwi_oper_add(gwi, opck_array))
   GWI_B(gwi_oper_emi(gwi, opem_array_access))
   GWI_B(gwi_oper_end(gwi, "[]", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", NULL, "void"))
   GWI_B(gwi_oper_emi(gwi, opem_array_each_init))
   GWI_B(gwi_oper_end(gwi, "@each_init", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", NULL, "int"))
   GWI_B(gwi_oper_emi(gwi, opem_array_each))
   GWI_B(gwi_oper_end(gwi, "@each", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", NULL, NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_each_val))
   GWI_B(gwi_oper_end(gwi, "@each_val", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", NULL, "int"))
   GWI_B(gwi_oper_end(gwi, "@each_idx", NULL))
   GWI_B(gwi_oper_ini(gwi, "Array", NULL, NULL))
   GWI_B(gwi_oper_add(gwi, opck_array_scan))
   GWI_B(gwi_oper_end(gwi, "class", NULL))

   GWI_B(gwi_oper_ini(gwi, (m_str)OP_ANY_TYPE, NULL, "bool"))
