...
/*      f1 = namespace_lookup_func(env->curr, binary->rhs->primary_exp->var, -1);*/
  ...
  if(r_nspc && !l_nspc)
  {
    ...
    err_msg(TYPE_, 0, "not match found for function '%s'", S_name(f2->def->name));
    return NULL;
  }
    // check for arrays
...
/*      v = namespace_lookup_value(env->curr, func_call->func->primary_exp->var, 1);*/
    else
    {
      ...
      v = find_value(func_call->func->dot_member->t_base, func_call->func->dot_member->xid);
    }
    // primary case
...
static m_bool check_For(Env env, Stmt_For stmt)
{
  ...
  default:
      ...
      err_msg(EMIT_,  stmt->c2->stmt_exp->pos,
          "invalid type '%s' in for condition", stmt->c2->stmt_exp->type->name);
      return -1;
  }
...
static m_bool check_Stmt(Env env, Stmt* stmt) {
  ...
  switch( stmt->type )
  {
    case ae_stmt_exp:
      if(stmt->stmt_exp)
        ret = (check_Expression(env, stmt->stmt_exp) ? 1 : -1);
      break;
    case ae_stmt_code:
      env->class_scope++;
      ret = check_Stmt_Code( env, stmt->stmt_code, 1);
      env->class_scope--;
      break;
    case ae_stmt_return:
      ret = check_Return(env, stmt->stmt_return);
      break;
    case ae_stmt_break:
      ret = check_Break(env, stmt->stmt_break);
      break;
    case ae_stmt_continue:
      ret = check_Continue(env, stmt->stmt_continue);
      break;
    case ae_stmt_if:
      env->class_scope++;
      namespace_push_value(env->curr);
    ...
    case ae_stmt_switch:
      ...
      break;
    case ae_stmt_case:
      ret = check_Case(env, stmt->stmt_case);
      break;
    case ae_stmt_enum:
      ret = check_Enum(env, stmt->stmt_enum);
      break;
    case ae_stmt_gotolabel:
      ret = check_Goto_Label(env, stmt->stmt_gotolabel);
      break;
    case ae_stmt_funcptr:
      ret = check_Func_Ptr(env, stmt->stmt_funcptr);
      break;
    case ae_stmt_union:
      l = stmt->stmt_union->l;
      if(env->class_def)
