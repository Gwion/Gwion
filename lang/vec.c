...
INSTR(vec3_add)
{
	...
	r.y = t->y + (t+1)->y;
	r.z = t->z + (t+1)->z;
	*(m_float*)shred->reg = r.x; shred->reg += SZ_FLOAT;
	*(m_float*)shred->reg = r.y; shred->reg += SZ_FLOAT;
...
INSTR(vec4_add)
{
	...
	*(m_float*)shred->reg = r.y; shred->reg += SZ_FLOAT;
	*(m_float*)shred->reg = r.z; shred->reg += SZ_FLOAT;
	*(m_float*)shred->reg = r.w; shred->reg += SZ_FLOAT;
}
