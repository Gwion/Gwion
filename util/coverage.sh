...
_fancy_output() {
	...
	printf "\b\b\b\b\b\b\b\b\b\b\b\b\b\b\b %i seconds $%s " "$COUNT" "$1"
}
fancy_output() {
    local COUNT="0.0";
...
#	grep 'Invalid read  of size ' "../log/$1.log" > /dev/null && mv "$1" ../leak
	...
	grep 'All heap blocks were freed -- no leaks are possible' "../log/$1.log"> /dev/null && rm "../log/$1.log" && return
